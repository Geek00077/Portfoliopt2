...
const {
    getSlugcats,    // GET    — fetch all notes belonging to the authenticated user
    setSlugcat,     // POST   — create a new note
    updateSlugcat,  // PUT    — overwrite an existing note by ID
    deleteSlugcat   // DELETE — remove a note by ID
} = require('../controllers/SlugcatController')
...
router.get('/', getSlugcats)
router.post('/',protect, setSlugcat)
...
router.route('/:id').put(protect, updateSlugcat).delete(protect, deleteSlugcat)
